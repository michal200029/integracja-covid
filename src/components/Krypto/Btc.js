...
export class Btc extends Component {
    ...
    componentDidMount(){
        ...
        axios.post(`http://localhost:8080/crypto`,{
            base:"BTC",
            startDate:"2020-01-01",
            endDate:"2020-08-01"
        },{headers})
        .then(res => {this.setState({values : res.data.currencyData})
        if(this.state.values[0]?.date)
        {
            this.setState({feb:this.state.values[2]})
            this.setState({apr:this.state.values[6]})
        }
    }) 
    }
                    ...
                    <Row xs={1} md={2} className="justify-content-md-center">
                        <Col>
                            <Card className="center">
                                <Card.Img src="https://pagesix.com/wp-content/uploads/sites/3/2021/06/covid-19.jpg?quality=75&strip=all" alt="my image" />
                                <Card.Title>BTC</Card.Title>
                                <Card.Body>
                                    <Card.Text>Bitcoin, bitmoneta – kryptowaluta wprowadzona w 2009 roku przez osobę (bądź grupę osób) o pseudonimie Satoshi Nakamoto[2]. Nazwa ta odnosi się także do używającego jej otwartoźródłowego oprogramowania węzłów tworzących sieć typu peer-to-peer. Od 2009 do 2021 roku oprogramowanie serwisu (Bitcoin Core) pobrano ponad 5,5 mln razy. W marcu 2017 roku Polska znajdowała się na 9. miejscu na liście z liczbą ponad 126 tys. pobrań. Prywatne klucze kryptograficzne, których używa się do autoryzowania transakcji, mogą zostać zapisane na komputerze osobistym w stworzonym do tego celu oprogramowaniu, w przeznaczonej do tego aplikacji na smartfonie lub w tzw. portfelu sprzętowym.</Card.Text>
                                </Card.Body>
                                <Stack>
                                            ...
                                            <TableBody>
                                                {
                                                    values.map(d => (
                                                        ...
                                                        >
                                                            ...
                                                            <TableCell align="center">{d.rate.toFixed(2)}</TableCell>
                                                        </TableRow>
                                                    )
                                                    )
                                                }
                                        ...
                                        <Table sx={{ minWidth: 700 }} aria-label="spanning table">
                                            ...
                                            <TableBody>
                                                ...
                                                <StyledTableRow>
                                                    ...
                                                    <TableCell align="center">{feb.rate.toFixed(2)}</TableCell>
                                                    <TableCell align="center">{apr.rate.toFixed(2)}</TableCell>
                                                </StyledTableRow>
                                                <TableRow>
                                                    <TableCell rowSpan={3} />
                                                    <TableCell align="center" >Różnica w wartościach</TableCell>
                                                    <TableCell align="center">{(feb.rate - apr.rate).toFixed(1)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell align="center">Procentowy ubytek</TableCell>
                                                    <TableCell align="center">{((1 - (apr.rate / feb.rate)) * 100).toFixed(2)}%</TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
                                    </TableContainer>
